...
export default function Dashboard() {
  ...
  const salesData = data?.trends?.sales?.map(i => ({ name: MON[i._id.month - 1], value: i.total }))
    || Array.from({ length: 7 }, (_, i) => ({ name: MON[i], value: 20000 + Math.random() * 70000 }));

  const expenseData = data?.trends?.expenses?.map(i => ({ name: MON[i._id.month - 1], value: i.total }))
    || Array.from({ length: 7 }, (_, i) => ({ name: MON[i], value: 8000 + Math.random() * 35000 }));
